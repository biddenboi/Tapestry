...
src/app owns startup and coordination. App.jsx creates DatabaseConnection, exposes AppContext, tracks the active player, current game state, active match, active task, active panel, notifications, toasts, and data source readiness. DataSourceGate is the desktop folder picker and linked-folder readiness gate. MobileCloudBootstrapGate is the clean-device mobile gate: it authenticates the owner, restores the bounded mobile-safe server snapshot into local SQLite, and then lets the operation log converge newer changes. NotificationCenter renders transient and persisted notifications. GameHub owns the main shell, floating map controls, map-first base layer, overlay panel routing, wake/sleep checks, persistent reminder toasts, profile switching, quick notes, inbox access, background task nudges, and lazy loading of feature panels.
...
The selected folder is canonical on desktop. Desktop startup asks DataSourceGate to establish access. DatabaseConnection loads .tapestry into memory, writes through the save bundle helpers, and keeps the folder synchronized. Routine writes are lightly debounced, major actions still force a folder write, and pending writes are flushed when the tab hides or the page unloads. The cached source handle exists only so refresh can reopen the same folder without repeatedly showing the picker. If the folder disappears or access is revoked, App.jsx clears readiness and returns to the folder screen. Mobile retains local SQLite as its authority but bootstraps a clean install from the owner-scoped server mirror; desktop publishes that mirror automatically when Private Sync connects, and Settings provides an explicit Publish mobile data action. The signed-in desktop can establish or change the private-sync password, which mobile uses directly without sending an authentication email. Magic links and ZIP import remain recovery routes rather than the normal phone setup path.

A root-level datetime zip snapshot is created on app open. Older app-generated snapshots are removed so only the newest automatic snapshot remains.
