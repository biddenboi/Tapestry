...
export const ACHIEVEMENT_PROCESSOR_VERSION = 1;
...
export const ACHIEVEMENT_EVENT_GROUPS = Object.freeze({
  [ACHIEVEMENT_EVENT_TYPE.taskCompleted]: Object.freeze([
    'grinder', 'scorer', 'deep_work', 'consistency', 'scholar', 'basket',
    'king_of_the_hill', 'peace', 'savant',
  ]),
  [ACHIEVEMENT_EVENT_TYPE.matchCompleted]: Object.freeze([
    'overkill', 'underdog', 'contributor', 'soldier', 'long_game', 'climber',
    'clutch', 'momentum', 'savant',
  ]),
  [ACHIEVEMENT_EVENT_TYPE.journalSaved]: Object.freeze(['legacy', 'basket']),
  [ACHIEVEMENT_EVENT_TYPE.timelineEventCreated]: Object.freeze(['basket']),
  [ACHIEVEMENT_EVENT_TYPE.eventLogged]: Object.freeze(['event_runner', 'fellowship']),
  [ACHIEVEMENT_EVENT_TYPE.inventoryChanged]: Object.freeze(['hobbyist', 'savant']),
  [ACHIEVEMENT_EVENT_TYPE.economyLogged]: Object.freeze(['treasurer']),
  [ACHIEVEMENT_EVENT_TYPE.socialChanged]: Object.freeze(['town']),
  [ACHIEVEMENT_EVENT_TYPE.profileUpdated]: Object.freeze(['signature', 'climber', 'savant']),
});
...
export async function processAchievementEvent(databaseConnection, eventOrId, context = {}) {
  ...
  return scheduleAchievementOperation(databaseConnection, event, async () => {
    ...
    const earnedKeys = Array.isArray(persistedAwards) ? persistedAwards : evaluation.earned;
    state = {
      ...state,
...
export function buildAchievementCountersFromRecords(player, data = {}) {
  ...
  return {
    ...
    fellowshipContribution: eventLogs
      ...
      .reduce((sum, entry) => sum + Math.max(0, finite(entry.multiplierValue, 1) - 1), 0),
...
export async function reconcileAchievementState(databaseConnection, player, {
  ...
  await databaseConnection.add(STORES.achievementState, state);
  const earned = await reconcileAchievements(player, databaseConnection, { reason, data: records });
  return { state, earned };
}
