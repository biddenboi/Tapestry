...
const reconcileAchievements = async () => ['reconciled'];`,
  );
...
test('ordinary processing reads only the player and compact achievement stores', async () => {
  ...
  assert.ok(result.earned.includes('grinder_1'));
  assert.ok(result.earned.includes('scorer_1'));
  assert.ok(result.earned.includes('deep_work_1'));
  assert.ok(result.earned.includes('basket_1'));
  ...
  assert.deepEqual([...new Set(db.getAllReads)], [STORES.achievementState]);
...
test('explicit reconciliation is guarded and can rebuild counters from supplied records', async () => {
  ...
  assert.deepEqual(db.getAllReads, [], 'supplied repair data must not be re-read');
});
