...
const getCompletedMatchIGT = (match) => {
  const completedIGT = Number(match?.completedInGameTimestamp ?? match?.result?.inGameTimestamp);
  if (Number.isFinite(completedIGT)) return Math.max(0, completedIGT);
  const startIGT = Number(match?.inGameTimestamp);
  return Number.isFinite(startIGT) ? Math.max(0, startIGT) : 0;
};
...
function estimateGhostByRate(player, durationHours, completedTasks) {
  ...
  const eloRate = (Math.max(100, player.elo || 900) / 1000) * (1.0 / 10000);
  const UTIL = 0.4;

  if (!completedTasks.length) {
    return {
      pointsPerMs: eloRate,
      estimatedTotal: Math.max(60, Math.round(eloRate * durationHours * HOUR * UTIL)),
      recentTaskNames: [],
    };
  }
  const totalDur = completedTasks.reduce((s, t) => s + getTaskDuration(t), 0);
  const totalPts = completedTasks.reduce((s, t) => s + Number(t.points || 0), 0);
  const rawRate = totalDur > 0 ? totalPts / totalDur : eloRate;
  const trust = Math.min(completedTasks.length / 10, 1);
  const ppm = Math.min(rawRate, MAX_RATE) * trust + eloRate * (1 - trust);
  return {
    pointsPerMs: ppm,
...
// 2-vs-3 brute-force balance over fixed n=5 (10 candidate splits).
function chooseBalancedTeams(currentPlayer, ghosts) {
  const scored = ghosts.map((g) => ({ ...g, matchPower: (g.elo || 1000) + (g.estimatedTotal || 0) * 0.55 }));
  const myPower = (currentPlayer.elo || 1000) + 220;
  let best = null;
  for (let i = 0; i < scored.length; i += 1) {
    for (let j = i + 1; j < scored.length; j += 1) {
      const t1 = [scored[i], scored[j]];
      const t2 = [];
      for (let k = 0; k < scored.length; k += 1) if (k !== i && k !== j) t2.push(scored[k]);
      if (t2.length !== 3) continue;
      const diff = Math.abs(
        t1.reduce((s, g) => s + g.matchPower, myPower) - t2.reduce((s, g) => s + g.matchPower, 0)
      );
      if (!best || diff < best.diff) best = { team1: t1, team2: t2, diff };
    }
  }
  return best || { team1: scored.slice(0, 2), team2: scored.slice(2, 5) };
}
...
export async function buildGhostRoster(db, allPlayers, currentPlayer, durationHours, options = {}) {
  ...
  const candidates = allPlayers.filter((p) => p.UUID !== currentPlayer.UUID);
  ...
  const myElo = currentPlayer.elo || 1000;
  rated.sort((a, b) => {
  ...
  // Synthetic "Echo" fillers are now rank-gated. At Iron/Bronze, the original
  // behaviour stands: pad to 5 with deterministic Echo clones. At Silver+,
  ...
  const { team1, team2 } = chooseBalancedTeams(currentPlayer, selected);
  return { teammates: team1, opponents: team2, insufficient: false };
}
...
export function buildGhostScoresSync(match, currentPlayerUUID, now = Date.now()) {
  ...
  for (const player of all) {
    ...
    next[player.UUID] = getGhostScore(player, match.createdAt, match.duration, now);
  }
  return next;
}
