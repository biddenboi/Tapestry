...
const source = (await readFile(new URL('./Match.js', import.meta.url), 'utf8'))
  .replace(
    ...
    "const HOUR = 60 * 60 * 1000; const STORES = { match: 'matches', task: 'tasks' };",
  )
  .replace(
    "import { getTaskDuration } from '@domain/tasks/Tasks.js';",
    "const getTaskDuration = (task) => Math.max(0, new Date(task.completedAt).getTime() - new Date(task.createdAt).getTime());",
  )
  .replace(
...
test('ghost roster estimates opponents from history visible at the match IGT', async () => {
  ...
  const players = [
    ...
    { UUID: 'p4', username: 'P4', elo: 1000 },
    { UUID: 'p5', username: 'P5', elo: 1000 },
  ];
  const matches = [
    {
      UUID: 'past-match',
