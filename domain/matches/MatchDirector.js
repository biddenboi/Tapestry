...
function pushEvent(events, recentEvents, now, payload, cooldownMs) {
  if (hasRecentEvent([...events, ...recentEvents], payload.dedupeKey, cooldownMs, now)) return;
  events.push(makeEvent(payload, now, events.length));
}
...
export function deriveMatchEvents(previousSnapshot, currentSnapshot, recentEvents = [], options = {}) {
  ...
  Object.keys(currentSnapshot.scoresByUUID || {}).forEach((uuid) => {
    ...
    pushEvent(events, recentEvents, now, {
      type: 'big_completion',
      severity,
      message: `${name} completed a big task for +${Math.round(delta).toLocaleString()}.`,
      teamIdx,
      playerUUID: uuid,
      points: Math.round(delta),
      taskName,
      ...timelineFields(currentSnapshot, currentActivity?.lastCompletedOffsetMs ?? currentSnapshot.elapsedMs),
  ...
  Object.entries(currentSnapshot.playerStatesByUUID || {}).forEach(([uuid, activity]) => {
    ...
    pushEvent(events, recentEvents, now, {
      ...
      severity: 'warning',
      message: `${getPlayerName(currentSnapshot, uuid)} is close to a +${Math.round(activity.pendingPoints).toLocaleString()} task.`,
      teamIdx,
      playerUUID: uuid,
      ...timelineFields(currentSnapshot, activity?.activeSessionEndOffsetMs ?? currentSnapshot.elapsedMs),
