...
const persistence = await readFile(new URL('../../data/db/saveBundleFormat.js', import.meta.url), 'utf8');
...
test('post-match jobs and receipts are durable, idempotent, and recoverable', () => {
  assert.match(persistence, /backgroundJobs\.json/);
  assert.match(persistence, /backgroundJobReceipts\.json/);
