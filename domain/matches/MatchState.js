import { HOUR } from '@domain/constants.js';
...
export function buildMatchSnapshot({
  ...
  const durationMs = Math.max(1, toFiniteNumber(match?.duration) * HOUR);
  const matchCreatedAtMs = new Date(match?.createdAt || 0).getTime();
