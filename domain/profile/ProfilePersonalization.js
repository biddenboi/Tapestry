...
export const PROFILE_THEME_SKINS = [
  ...
  },
  {
    ...
    requiredTheme: 'paper',
    layout: 'notebook',
    accent: THEME_ACCENT_COLORS.paper,
  ...
  },
  {
    ...
    requiredTheme: 'pure',
    layout: 'gallery',
    accent: THEME_ACCENT_COLORS.pure,
  ...
  },
  {
    ...
    requiredTheme: 'gold',
    layout: 'collector',
    accent: THEME_ACCENT_COLORS.gold,
  ...
  },
  {
    ...
    requiredTheme: 'crimson',
    layout: 'dossier',
    accent: THEME_ACCENT_COLORS.crimson,
...
const UNIQUE_PROFILE_BLOCK_TYPES = new Set(PROFILE_BLOCK_DEFINITIONS.filter((block) => !block.free).map((block) => block.type));
...
function normalizeProfileBlocks(blocks = []) {
  ...
  return rows.reduce((result, block, index) => {
    ...
    const legacyColumns = block.width
      ? (block.width === 'wide' ? 12 : 6)
      : definition?.defaultColumns;

    result.push({
      id: String(block.id || `${type}-${index + 1}`),
      type,
      columns: clampProfileBlockColumns(block.columns ?? legacyColumns),
...
function migrateLegacyBlocks(source, normalized) {
  const hasLegacySettings = ['showStats', 'showAchievements', 'showHighlights', 'showActivity', 'about', 'aboutMarkdown', 'quote', 'links', 'socialLinks']
    .some((key) => Object.prototype.hasOwnProperty.call(source, key));
  if (!hasLegacySettings) return [];

  const blocks = [];
  const aboutText = [normalized.quote, normalized.about].filter(Boolean).join('\n\n');
  if (aboutText || normalized.links.some((link) => link.label || link.url)) {
    const linksText = normalized.links
    ...
    blocks.push({
      ...
      height: 240,
      title: 'About',
      content: [aboutText, linksText].filter(Boolean).join('\n\n'),
    });
  }
  if (normalized.showStats) blocks.push({ id: 'legacy-stats', type: 'stats', columns: 6, height: 240 });
  blocks.push({ id: 'legacy-rank-graph', type: 'rankGraph', columns: 6, height: 360 });
  if (normalized.showAchievements) blocks.push({ id: 'legacy-achievements', type: 'achievements', columns: 6, height: 240 });
  if (normalized.showHighlights) blocks.push({ id: 'legacy-highlights', type: 'highlights', columns: 6, height: 280 });
  if (normalized.showActivity) blocks.push({ id: 'legacy-activity', type: 'activity', columns: 12, height: 380 });
  return blocks;
}
...
export function normalizeProfilePersonalization(raw = {}) {
  ...
  const next = {
    ...
    ...source,
    about: source.about ?? source.aboutMarkdown ?? DEFAULT_PROFILE_PERSONALIZATION.about,
    links: source.links ?? source.socialLinks ?? DEFAULT_PROFILE_PERSONALIZATION.links,
  };
  const normalized = {
  ...
  return {
    ...
    blocks: Array.isArray(source.blocks)
      ? normalizeProfileBlocks(source.blocks)
      : migrateLegacyBlocks(source, normalized),
  };
}
