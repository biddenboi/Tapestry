...
export function buildDojoRoomFactRequest({ scene, viewerIGT, dojoSessionUUID } = {}) {
  return selectDojoRoomMembers(scene, viewerIGT)
    .filter((member) => member.occupantKind !== 'resident')
    .map((member) => Object.freeze({
...
export function projectDojoRoomRows({
  ...
  return Object.freeze(selectDojoRoomMembers(scene, viewerIGT).map((member) => {
    if (member.occupantKind === 'resident') {
      return Object.freeze({
        profileId: asId(member.profileId),
        presenceIntervalId: member.presence.intervalId || null,
        identity: member.identity,
        role: member.role,
        occupantKind: member.occupantKind,
        timeBasis: member.timeBasis,
        residentCard: member.residentCard,
        presenceState: member.presence.state,
        elapsedHere: member.presence?.elapsedHere == null
          ? null
          : Math.max(0, finite(member.presence.elapsedHere)),
        focusedMs: null,
        sessionId: null,
        sessionPoints: null,
        taskLabel: null,
    ...
    const presenceFocused = projectFocused(member.presence, sceneIGT, viewerIGT, liveViewerPaused);
