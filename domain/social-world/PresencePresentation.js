...
export function buildPresencePresentation(presence, viewerIGT) {
  ...
  if (state === PRESENCE_STATE.current || state === PRESENCE_STATE.projected) {
    ...
    return Object.freeze({
      ...
      elapsedHereLabel,
      activeElapsedLabel,
      primary: [locationLabel, ...facts].filter(Boolean).join(' · '),
      secondary: presence.paused ? 'Productive time is paused; occupancy continues.' : null,
    });
  }
  ...
  if (state === PRESENCE_STATE.recent) {
    return Object.freeze({
      ...
      secondary: elapsedHereLabel ? `${elapsedHereLabel} total` : 'Duration unavailable from legacy evidence',
    });
  }
