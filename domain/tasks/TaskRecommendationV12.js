...
async function flushLinkedFolderGeneration(databaseConnection) {
  if (typeof databaseConnection.flushLinkedFolderWrite !== 'function') {
    return Object.freeze({ durable: true, reason: 'no-linked-folder-flush-api' });
  }
  try {
    const result = await databaseConnection.flushLinkedFolderWrite();
    const reason = result?.reason || 'flushed';
    return Object.freeze({
      durable: !['pending-retry', 'permission-required'].includes(reason),
      reason,
    });
  } catch (error) {
    return Object.freeze({ durable: false, reason: error?.message || String(error) });
  }
}

function importReceiptValue(value = {}, durability = null) {
  const durable = durability?.durable === true;
  return Object.freeze({
    ...value,
    status: durable ? 'complete' : 'pending-linked-folder',
    linkedFolderDurable: durable,
    recoveryRequired: !durable,
    recoveryReason: durable ? null : (durability?.reason || 'linked-folder-write-pending'),
  });
}

export async function readTaskRecommendationV12Checkpoint(databaseConnection, playerUUID) {
  await assertTaskRecommenderV12RuntimeReady(databaseConnection, playerUUID);
...
export async function trainTaskRecommendationV12(databaseConnection, input = {}) {
  ...
  await assertTaskRecommenderV12RuntimeReady(databaseConnection, request.playerUUID);
...
export async function reportTaskRecommendationV12Evidence(
  ...
  await assertTaskRecommenderV12RuntimeReady(databaseConnection, owner);
...
export async function exportTaskRecommendationV12Bundle(databaseConnection, playerUUID) {
  ...
  const [checkpoint, protocolEvents, migrationState, appSettings] = await Promise.all([
    getTaskRecommenderV12Checkpoint(databaseConnection, owner),
    getTaskRecommenderProtocolEvents(databaseConnection, owner),
    getTaskRecommenderV12MigrationState(databaseConnection, owner),
    databaseConnection.getPlayerStore(STORES.appSetting, owner).catch(() => []),
  ]);
  ...
  const evidencePayload = {
    ...
    migrationState: migrationState?.status || null,
  };
...
export async function resumeTaskRecommendationV12Import(
  ...
  const durability = await flushLinkedFolderGeneration(databaseConnection);
  return importReceiptValue(receipt.value, durability);
}
...
export async function rollbackTaskRecommendationV12Import(
  ...
  return Object.freeze({
    ...
    linkedFolderDurable: durability.durable,
    recoveryRequired: !durability.durable,
    recoveryReason: durability.durable ? null : durability.reason,
  });
}
