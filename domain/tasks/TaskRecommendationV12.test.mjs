...
class MemoryDb {
  ...
  async flushLinkedFolderWrite() {
    return this.flushResults.length ? this.flushResults.shift() : { reason: 'flushed' };
  }
}
...
test('v12 bundle import is atomic, resumes linked-folder durability, and rolls back only to v12 state', async () => {
  const db = new MemoryDb();
  ...
  assert.equal(imported.status, 'pending-linked-folder');
  ...
  assert.equal(resumed.linkedFolderDurable, true);
