...
test('policy payload reader preserves legacy v1 decision receipts', () => {
  const { valueModel, actions, recurrentState } = fixture();
  const current = policy.buildTaskRecommenderV12PolicyDecision({
    model: valueModel,
    recurrentState,
    actions,
    seed: 'legacy-reader',
  ...
  legacy.policySchemaVersion = 1;
  legacy.policyVersion = 'posterior-safe-v1';
  delete legacy.evidence;
  assert.deepEqual(policy.taskRecommenderV12PolicyDecisionPayload(legacy), legacy);
});
...
test('v1 evidence-free safety debt is quarantined while receipts remain auditable', async () => {
  const db = new MemoryDb();
  const { valueModel, actions, recurrentState } = fixture();
  const decision = policy.buildTaskRecommenderV12PolicyDecision({
    model: valueModel,
    recurrentState,
    actions,
    seed: 'legacy-budget-receipt',
  }).policyDecision;
  const legacyDecision = JSON.parse(JSON.stringify(decision));
  legacyDecision.policySchemaVersion = 1;
  legacyDecision.policyVersion = 'posterior-safe-v1';
  delete legacyDecision.evidence;
  await db.add('appSettings', {
    UUID: 'task-recommender-v12-policy-state:player-legacy',
    parent: 'player-legacy',
    value: {
      policyStateSchemaVersion: 1,
      budget: {
        safetyFraction: 0.1,
        balanceHours: -2.1,
        pending: [{
          decisionUUID: 'legacy-decision',
          reservedHours: 0,
          championReferenceHours: 2.3,
        }],
      },
      decisionReceipts: [{
        decisionUUID: 'legacy-decision',
  ...
  const migrated = await policyState.getTaskRecommenderV12PolicyState(db, 'player-legacy');
  assert.equal(migrated.migratedLegacySafetyBudget, true);
  assert.equal(migrated.state.policyStateSchemaVersion, 4);
  assert.equal(migrated.state.budget.balanceImmediateWorkHours, 0);
  assert.equal(migrated.state.budget.pending.length, 0);
  assert.equal(migrated.state.decisionReceipts.length, 1);
  assert.equal(migrated.state.decisionReceipts[0].policyDecision.policyVersion, 'posterior-safe-v1');
  assert.equal(migrated.state.legacyBudgetMigration.discardedBalanceHours, -2.1);
});

test('v2 long-horizon safety balances are quarantined', async () => {
  const db = new MemoryDb();
  await db.add('appSettings', {
    UUID: 'task-recommender-v12-policy-state:player-v2',
    parent: 'player-v2',
    value: {
      policyStateSchemaVersion: 2,
      budget: {
        budgetSchemaVersion: 1,
        balanceHours: 3.5,
        pending: [{
          decisionUUID: 'old-units',
          reservedHours: 0.4,
          championReferenceHours: 1.2,
        }],
      },
      decisionReceipts: [],
    },
  });
  const migrated = await policyState.getTaskRecommenderV12PolicyState(db, 'player-v2');
  assert.equal(migrated.state.policyStateSchemaVersion, 4);
  assert.equal(migrated.state.budget.balanceImmediateWorkHours, 0);
  assert.equal(migrated.state.budget.pending.length, 0);
  assert.equal(migrated.state.legacyBudgetMigration.fromSchemaVersion, 2);
  assert.equal(
    migrated.state.legacyBudgetMigration.reason,
    'long-horizon-safety-budget-quarantined',
  );
  assert.equal(migrated.state.legacyBudgetMigration.discardedBalanceHours, 3.5);
});
