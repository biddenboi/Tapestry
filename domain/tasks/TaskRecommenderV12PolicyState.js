...
export function normalizeTaskRecommenderV12PolicyState(value = {}) {
  ...
  return Object.freeze({
    ...
    legacyBudgetMigration: value.legacyBudgetMigration
      ? Object.freeze({ ...value.legacyBudgetMigration })
      : null,
  });
}
...
export async function getTaskRecommenderV12PolicyState(databaseConnection, playerUUID) {
  ...
  try {
    ...
    if (schemaVersion === 1 || schemaVersion === 2) {
      const legacyBudget = normalizeTaskRecommenderV12BudgetState(record.value.budget);
      return {
        state: normalizeTaskRecommenderV12PolicyState({
          ...record.value,
          budget: {},
          legacyBudgetMigration: {
            fromSchemaVersion: schemaVersion,
            reason: schemaVersion === 1
              ? 'evidence-free-champion-budget-quarantined'
              : 'long-horizon-safety-budget-quarantined',
            discardedBalanceHours: Number(record.value.budget?.balanceHours) || 0,
            discardedPendingCount: legacyBudget.pending.length,
            preservedReceiptCount: Array.isArray(record.value.decisionReceipts)
              ? record.value.decisionReceipts.length
              : 0,
          },
        }),
        recoveredFromInvalidState: false,
        migratedLegacySafetyBudget: true,
      };
    }
    if (schemaVersion !== 3 && schemaVersion !== TASK_RECOMMENDER_V12_POLICY_STATE_SCHEMA_VERSION) {
