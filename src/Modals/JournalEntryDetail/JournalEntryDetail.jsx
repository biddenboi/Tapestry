    ...
    <div className="modal-blanker">
      <div className="modal-card jed-card">
        ...
        <div className="jed-header">
          {editing
            ? <input
                className="jed-title-input"
                value={title}
                onChange={e => setTitle(e.target.value)}
                placeholder="Entry title"
                autoFocus
              />
            : <span className="modal-title">{title || 'Journal entry'}</span>
          }
          <div className="jed-header-actions">
            ...
            {!editing && (
              <button className="btn-danger jed-delete-btn" onClick={handleDelete}>Delete</button>
            )}
            <button className="btn-ghost jed-close" onClick={() => { modal.hide(); modal.remove(); }}>&#10005;</button>
          </div>
        </div>

        <span className="label-sm jed-date">
          {UTCStringToLocalDate(entry.createdAt)} &middot; {UTCStringToLocalTime(entry.createdAt)}
        </span>
        ...
        {editing ? (
          ...
          <div className="jed-body">
            {text
              ? text.split('\n').map((line, i) => (
                  <p key={i} className="jed-line">{line || <br />}</p>
                ))
              : <span className="jed-empty">No content.</span>
            }
          </div>
        )}
      </div>
