...
import { THEME_REGISTRY, COSMETIC_TITLES, COSMETIC_PASSES, SPECIAL_KIND, STORES } from '@domain/constants.js';
...
export const TOTAL_PAID_COSMETICS =
  THEME_REGISTRY.filter((t) => !t.free).length +
...
const ACHIEVEMENT_GROUP_META = {
  ...
  fellowship: {
    name: 'Fellowship',
    category: 'events',
    summary: 'Complete day-close rituals consistently.',
  },
  treasurer: {
    name: 'Treasurer',
    category: 'economy',
...
export function getAchievementByKey(key) {
  if (ACHIEVEMENT_MAP[key]) return ACHIEVEMENT_MAP[key];
  const current = ACHIEVEMENT_V2_BY_ID.get(key);
  ...
  return {
    ...
    color: colorByCategory[current.category] || '#7ca7d8',
    icon: svgWrap('<path d="M10 2.5 16.5 6v8L10 17.5 3.5 14V6Z"/><path d="m7 10 2 2 4-4"/>'),
...
function lifetimePoints(tasks, playerUUID) {
  return tasks
    ...
    .reduce((s, t) => s + getCanonicalTaskPoints(t), 0);
}
...
export async function reconcileAchievements(player, db, { reason, data = null } = {}) {
  ...
  allTasks.filter((t) => t.completedAt && t.parent && t.source === 'dojo').forEach((t) => {
    ...
    dayMap[key].points += getCanonicalTaskPoints(t);
  });
