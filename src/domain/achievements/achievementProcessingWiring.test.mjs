...
test('opening the achievements modal reads compact v2 evidence; broad reconciliation requires its explicit action', () => {
  const reconcileHandlerIndex = modal.indexOf('const reconcile =');
  ...
  assert.match(ordinaryLoad, /achievementV2\.getEvidence/);
  assert.match(ordinaryLoad, /achievementV2\.getAllProgress/);
  ...
  assert.match(processing, /tasks: await databaseConnection\.getAll\(STORES\.task\)/);
  assert.match(processing, /journals: await databaseConnection\.getAll\(STORES\.journal\)/);
});
