...
export function buildDojoLeaderboardSnapshot(tasks = [], { generatedAt = null, players = [] } = {}) {
  ...
  for (const task of tasks || []) {
    ...
    current.points += getCanonicalTaskPoints(task);
    current.taskCount += 1;
...
function normalizeReadableSnapshot(value = null) {
  return isCurrentSnapshot(value) ? value : emptyDojoLeaderboardSnapshot();
}
...
export async function recordDojoCompletionInSnapshot(databaseConnection, { task, event, player } = {}) {
  ...
  current.points = Math.floor(Number(current.points || 0)) + getCanonicalTaskPoints(task);
