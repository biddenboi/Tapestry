...
import { getMatchTeams, isPairMatch } from '@domain/matches/MatchContracts.js';
...
export function buildSnapshotEloChanges(match) {
  ...
  const calculate = isPairMatch(match)
    ? computePairMatchEloChanges
    : computeLegacyEloChanges;
  const raw = calculate(
    teams,
...
async function processElo(databaseConnection, matchUUID) {
  return withMatchMutationLock(databaseConnection, matchUUID, async () => {
    ...
    if (!isRatedMatch(match)) return { updated: false, unrated: true };
...
async function processAchievements(databaseConnection, matchUUID, context = {}) {
  ...
  const teammateUUID = teams[viewerTeamIdx]
    ?.find((participant) => String(participant.UUID) !== String(match.parent))
    ?.UUID || null;
  ...
  const event = createAchievementEvent({
    ...
    payload: {
      ...
      teamScore: viewerTeamScore,
      teammateUUID,
      fixedRuleset: match.mode !== 'practice',
      settled: match.result.validity?.validForRating !== false,
      expectedWinProbability: match.result.expectedWinProbability ?? match.expectedWinProbability ?? null,
      maxDeficitRecovered: finite(match.result.maxDeficitRecovered),
...
function contributionSummary(match) {
  ...
  return { version: 2, totalPoints: total, mvpUUID, byPlayer };
}
...
async function processContribution(databaseConnection, matchUUID) {
  return withMatchMutationLock(databaseConnection, matchUUID, async () => {
    const match = await databaseConnection.get(STORES.match, matchUUID);
    if (!match?.result || !match.parent) return { updated: false };
    const player = await databaseConnection.get(STORES.player, match.parent);
    if (match.result.validity?.validForRewards === false) {
      return { updated: false, ineligible: true, reason: match.result.validity.reason };
    }
    const contribution = player ? await recordActionContribution(databaseConnection, player, {
      source: 'match',
      sourceUUID: match.UUID,
      summary: match.result.iWon ? 'Completed match · win' : 'Completed match',
      createdAt: match.result.concludedAt || match.completedAt || new Date().toISOString(),
      inGameTimestamp: match.completedInGameTimestamp ?? match.result.inGameTimestamp ?? null,
    }) : null;

    const alreadySummarized = match.result.contributionSummary?.version === 2;
    if (!alreadySummarized) {
      ...
      return { updated: true, mvpUUID: summary.mvpUUID, contribution: contribution?.value || 0 };
    }
    return { updated: !!contribution, alreadyApplied: true, contribution: contribution?.value || 0 };
  });
}
