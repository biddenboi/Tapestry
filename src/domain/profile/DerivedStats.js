...
export function buildProfileStatsFromRecords(records, player, viewerIGT) {
  ...
  for (const task of tasks) {
    ...
    taskDaily[key].points += getCanonicalTaskPoints(task);
  ...
  const recent7Points = recentTasks.reduce((sum, task) => sum + getCanonicalTaskPoints(task), 0);
  const baselineDailyPoints = baselineTasks.reduce((sum, task) => sum + getCanonicalTaskPoints(task), 0) / 23;
  ...
  const sortedMatches = [...matches].sort((a, b) => (
    completedRecordIGT(a) - completedRecordIGT(b)
  ));
  const eloSeries = sortedMatches
    .filter((match) => match.result?.newElo != null)
    .map((match) => ({
      x: completedRecordIGT(match),
  ...
  return {
    ...
    taskStats: {
      total: tasks.length,
      totalPoints: tasks.reduce((sum, task) => sum + getCanonicalTaskPoints(task), 0),
