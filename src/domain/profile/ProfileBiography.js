...
const igtOf = (record) => completedRecordIGT(record);
...
export function buildProfileMilestones(profileView, rawRecords = {}, options = {}) {
  ...
  const highestTask = tasks.reduce((best, entry) => (
    !best
      || Number(entry.pointsBase ?? entry.points ?? 0) > Number(best.pointsBase ?? best.points ?? 0)
      ? entry
      : best
  ), null);
  if (highestTask) rows.push(milestone(
    ...
    'highestValueTask',
    'Highest-Value Task',
    `${highestTask.name || 'A task'} produced ${Math.floor(Number(highestTask.pointsBase ?? highestTask.points) || 0).toLocaleString()} points.`,
