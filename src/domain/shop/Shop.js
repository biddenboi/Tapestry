...
/** Stable catalog order using the canonical configured field. */
export function compareShopCatalogItems(left, right) {
  const leftOrder = finiteOrder(left?.displayOrder);
  const rightOrder = finiteOrder(right?.displayOrder);
...
export function isCosmeticItem(item) {
  return String(item?.type || '').startsWith('cosmetic_');
}

export function isConsumableInventoryItem(item) {
  return Boolean(item) && !isCosmeticItem(item);
}

export function findInventoryForShopItem(inventory = [], item = null) {
  if (!item) return null;
  return inventory.find((entry) => String(entry.itemUUID || '') === String(item.UUID || ''))
