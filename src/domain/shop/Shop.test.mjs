...
test('catalog ordering is deterministic and honors configured display order', () => {
  const input = [
    ...
    { UUID: 'focus', name: 'Focus', category: 'Focus', displayOrder: 15 },
    { UUID: 'a2', name: 'Alpha', category: 'Rest' },
  ];
  const ordered = shop.sortShopCatalog(input);

  assert.deepEqual(ordered.map((item) => item.UUID), ['a', 'focus', 'b', 'a2', 'z']);
  assert.deepEqual(input.map((item) => item.UUID), ['z', 'b', 'a', 'focus', 'a2']);
});
...
test('mobile inventory classification excludes cosmetics and resolves owned catalog items', () => {
  const catalogItem = { UUID: 'shop-title-gold', type: 'cosmetic_title', itemId: 'gold', name: 'Gold' };
  const cosmetic = { UUID: 'owned-title', itemUUID: 'shop-title-gold', type: 'cosmetic_title', itemId: 'gold', name: 'Gold' };
  const consumable = { UUID: 'owned-focus', itemUUID: 'shop-focus', type: 'duration', name: 'Focus', quantity: 2 };
  assert.equal(shop.isCosmeticItem(cosmetic), true);
  assert.equal(shop.isConsumableInventoryItem(cosmetic), false);
  assert.equal(shop.isConsumableInventoryItem(consumable), true);
  assert.equal(shop.findInventoryForShopItem([consumable, cosmetic], catalogItem), cosmetic);
});
