...
class MemoryDatabase {
  ...
  async commitAtomicMutation(input) {
    ...
    if (input.operationId && this.receipts.has(input.operationId)) {
      return { changed: false, duplicate: true, operationId: input.operationId };
    }
    if (input.operationId) this.receipts.add(input.operationId);
...
test('retrying a purchase batch returns the canonical receipt without spending twice', async () => {
  const player = { UUID: 'player-1', tokens: 20 };
  const db = new MemoryDatabase({ player, catalog: [tokenItem], inventory: [], money: 0 });
  const input = {
    playerUUID: player.UUID,
    purchaseBatchUUID: 'purchase-retry',
    now: '2026-08-02T10:00:00.000Z',
    uuidFactory: uuidSequence('retry'),
    cart: [{ item: tokenItem, qty: 1 }],
  };

  const first = await service.commitShopPurchase(db, input);
  const second = await service.commitShopPurchase(db, input);

  assert.equal(first.duplicate, false);
  assert.equal(second.duplicate, true);
  assert.equal(db.atomicCommits.length, 1);
  assert.equal((await db.get(STORES.player, player.UUID)).tokens, 10);
  assert.equal((await db.getAll(STORES.transaction)).length, 1);
});
