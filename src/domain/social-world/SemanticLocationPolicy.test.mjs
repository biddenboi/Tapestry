...
test('semantic location precedence keeps nested work inside Match and Dojo', () => {
  const scenarios = [
    ...
    },
    {
      ...
      input: { gameState: 'idle', activeTask: { createdAt: 'now' }, activePanel: 'tasks' },
      expected: SEMANTIC_LOCATION.taskSession,
    },
    {
      name: 'queue is planning',
      input: { gameState: 'idle', activePanel: 'queue' },
      expected: SEMANTIC_LOCATION.planning,
    },
    {
      name: 'shop is marketplace',
      input: { gameState: 'idle', activePanel: 'shop' },
      expected: SEMANTIC_LOCATION.marketplace,
    },
    {
      name: 'the idle app without stronger context is Commons',
      input: { gameState: 'idle' },
...
test('ordinary idle panels remain in Commons instead of moving the player Away', () => {
  for (const activePanel of [null, 'feed', 'events', 'inventory', 'pass', 'profile']) {
    assert.equal(resolveSemanticLocation({
      gameState: 'idle',
      activePanel,
    }), SEMANTIC_LOCATION.commons, activePanel || 'map');
  }
});
