...
function candidateStrength(manifest = {}) {
  const validationError = Number(manifest.trainingEvidence?.metrics?.validation?.meanSquaredError);
  if (Number.isFinite(validationError)) return -validationError;
  const improvement = Number(manifest.trainingEvidence?.metrics?.improvement);
  if (Number.isFinite(improvement)) return improvement;
  return Number.NEGATIVE_INFINITY;
}

/**
 * Keep only model weights that can affect serving or recovery: the active
 * champion, one rollback champion, and the strongest unpromoted candidate.
 * The mutable training checkpoint has its own fixed app-setting key and is
 * overwritten by saveTaskRecommenderV12Checkpoint.
 */
export async function pruneTaskRecommenderV12StoredModels(databaseConnection, playerUUID) {
  if (typeof databaseConnection?.getAll !== 'function') return { removed: 0, retained: [] };
  const owner = String(playerUUID);
  const pointer = await getRecord(databaseConnection, taskRecommenderV12ChampionPointerId(owner));
  const champion = pointer?.value?.championPolicyUUID || null;
  const previous = pointer?.value?.previousChampionPolicyUUID || null;
  const recordPrefix = `${TASK_RECOMMENDER_V12_POLICY_MANIFEST_PREFIX}:${owner}:`;
  const records = (await databaseConnection.getAll(STORES.appSetting))
    .filter((record) => String(record?.UUID || '').startsWith(recordPrefix));
  const protectedIds = new Set([champion, previous].filter(Boolean).map(String));
  const bestCandidate = records
    .map((record) => record.value)
    .filter((manifest) => (
      ['candidate', 'ablation'].includes(manifest?.role)
      && !protectedIds.has(String(manifest.policyUUID))
    ))
    .sort((left, right) => (
      candidateStrength(right) - candidateStrength(left)
      || String(right.createdAt || '').localeCompare(String(left.createdAt || ''))
    ))[0] || null;
  if (bestCandidate?.policyUUID) protectedIds.add(String(bestCandidate.policyUUID));
  const stale = records.filter((record) => !protectedIds.has(String(record.value?.policyUUID)));
  if (stale.length && typeof databaseConnection.commitAtomicMutation === 'function') {
    await databaseConnection.commitAtomicMutation({
      label: 'task-recommender-v12-model-retention',
      deletes: stale.map((record) => ({ store: STORES.appSetting, UUID: record.UUID })),
    });
  } else {
    for (const record of stale) await databaseConnection.remove(STORES.appSetting, record.UUID);
  }
  return { removed: stale.length, retained: [...protectedIds] };
}
...
export async function getTaskRecommenderV12PolicyManifest(
  ...
  if (Number(value?.policyManifestSchemaVersion)
    ...
    || value.runtime !== 'v12'
    || value.planningSemantics !== 'verified-work-only'
...
export async function ensureTaskRecommenderV12PolicyRegistry(
  ...
  const existingChampion = existingPointer?.value?.championPolicyUUID
    ...
    : null;
  if (existingChampion) {
    await pruneTaskRecommenderV12StoredModels(databaseConnection, owner);
...
export async function registerTaskRecommenderV12PolicyCandidate(
  ...
  await pruneTaskRecommenderV12StoredModels(databaseConnection, playerUUID);
  return manifest;
}
