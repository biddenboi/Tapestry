import { DAY, HOUR } from '@domain/constants.js';
...
// ─── Time-based task points ─────────────────────────────────────────────
// Universal task Points measure only active time. Competitive Match scoring
// owns its own explicit multiplier policy; normal work never inherits it.
export function getTimeBasedTaskPoints(durationMs = 0) {
  const safeDurationMs = Math.max(0, Number(durationMs) || 0);
  const exactBasePoints = safeDurationMs / 10000;
  if (exactBasePoints <= 0) return { points: 0, basePoints: 0, randomFactor: 1 };
  return {
    points: Math.floor(exactBasePoints),
    basePoints: Math.floor(exactBasePoints),
    randomFactor: 1,
  };
}

/** Whole direct-work points for persisted task history (never reward multipliers). */
export function getCanonicalTaskPoints(task = {}) {
  const storedBase = Number(task?.pointsBase);
  const storedPoints = Number(task?.points);
  // Pre-backfill typed rows used zero as a default even when their legacy
  // document carried Points. Prefer that legacy value only for this exact
  // sentinel shape; new zero-point work stores zero in both fields.
  const candidate = Number.isFinite(storedBase)
    && (storedBase > 0 || !Number.isFinite(storedPoints) || storedPoints === 0)
    ? storedBase
    : storedPoints;
...
export function createTaskDraft({
  ...
  return normalizeTaskPlanningMetadata({
    UUID: null,
    name: '',
    dueDate,
    estimatedDuration,
    ...
    nextAction: '',
    needsPlanning: false,
    taskRevisionHash: hashTaskRevision({
      name: '',
      description: '',
      dueDate,
      estimatedDuration,
