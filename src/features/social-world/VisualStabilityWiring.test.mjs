...
const [lobby, lobbyStyles, socialWorld, socialRuntime, overlayFocus, presetSurface, matchStyles] = await Promise.all([
...
test('prepared Fellow scenes stay mounted between factual source refreshes', () => {
  ...
  assert.doesNotMatch(`${socialRuntime}\n${lobby}`, /useSocialOccupancy|socialOccupancy|occupancy &&/);
});
