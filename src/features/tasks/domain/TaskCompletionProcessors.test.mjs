...
source = source
  .replace("import { STORES } from '@domain/constants.js';", `const STORES = {
...
const processAchievementEvent = async () => { globalThis.__taskProcessorCounters.achievements += 1; return { earned: ['earned'] }; };`)
  .replace("import { recordTaskContribution } from '@domain/contribution/Contribution.js';", "const recordTaskContribution = async (_db, _player, _task, _reward, options) => { globalThis.__taskProcessorCounters.contribution += 1; return { UUID: options.completionEventUUID, value: 2 }; };")
