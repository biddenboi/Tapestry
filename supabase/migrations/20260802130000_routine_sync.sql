-- Incremental routine synchronization. Kept as a narrow RPC so the original
-- command registry remains explicit and existing deployments upgrade safely.

create or replace function public.apply_routine_sync_batch(p_operations jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if jsonb_typeof(p_operations) <> 'array' then
    raise exception 'Routine operations must be a JSON array.' using errcode = '22023';
  end if;
  if jsonb_array_length(p_operations) > 100 then
    raise exception 'A routine batch cannot contain more than 100 operations.' using errcode = '22023';
  end if;
  ...
  loop
    ...
    begin
      ...
      if v_operation_id = '' or char_length(v_operation_id) > 500
         ...
         or v_command_type not in ('startRoutineRun','completeRoutineStep','completeRoutineRun')
         or (v_command_type = 'completeRoutineStep' and v_entity_type <> 'routine-step')
         or (v_command_type <> 'completeRoutineStep' and v_entity_type <> 'routine-run') then
        raise exception 'The routine operation has invalid required fields.' using errcode = '22023';
      end if;
      ...
      if v_command_type = 'completeRoutineStep' then
        if jsonb_typeof(v_payload -> 'run') <> 'object'
           or coalesce(v_payload ->> 'stepId','') = '' then
          raise exception 'A routine step requires its canonical run and step id.' using errcode = '22023';
        end if;
        ...
        if jsonb_typeof(v_payload -> 'run') <> 'object' then
          raise exception 'A routine command requires its canonical run.' using errcode = '22023';
        end if;
        select * into v_current from public.sync_entities
        where owner_id=v_owner_id and entity_type='routine-run' and entity_id=v_entity_id
        for update;
        if v_base_version is not null
          ...
          v_result := jsonb_build_object(
            ...
            'errorMessage','The server has a newer routine version.',
          ...
          insert into public.sync_entities(
            ...
            v_owner_id,'routine-run',v_entity_id,v_player_id,v_next_version,
            v_payload -> 'run',v_occurred_at,v_device_id,null
          )
          ...
          v_result := jsonb_build_object(
            ...
            'entity',jsonb_build_object(
              'entityType','routine-run','entityId',v_entity_id,
              'version',v_next_version,'data',v_payload -> 'run','deletedAt',null
            )
          );
        end if;
      end if;
...
revoke all on function public.apply_routine_sync_batch(jsonb) from public, anon;
grant execute on function public.apply_routine_sync_batch(jsonb) to authenticated;
