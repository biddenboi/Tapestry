-- Habits and Rhythms are shared workspace data. Keep their definitions,
-- evidence, and opportunity state inside the same durable reference mirror as
-- Tasks, Goals, Matches, and Chronicle data.
...
alter table public.mobile_reference_records
  add constraint mobile_reference_type check(record_type in (
    ...
    'routine-run','routine-step-receipt','effect-interval','effect-cancellation'
  ));
